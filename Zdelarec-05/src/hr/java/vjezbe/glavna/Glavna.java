...
import hr.java.vjezbe.entitet.Drzave;
...
public class Glavna {
	...
	public static Drzave<Drzava> drzave = new Drzave<>();
	...
	public static void main(String[] args) throws InterruptedException {
		...
		for (int i = 0; i < BROJ_POSTAJA; i++) {
			...
			if (i < 2) {
				mjernePostaje.add(dohvatiPostaju(novaMjernaPostaja, novoMjesto, novaGeografskaTocka, listaSenzora,
						nazivMjernePostaje));
			} else {
				mjernePostaje.add(new RadioSondaznaMjernaPostaja(nazivMjernePostaje, novoMjesto,
						novaGeografskaTocka, listaSenzora, visinaRadioSonazneMjernePostaje));
			}
			System.out.println(mjernePostaje.get(i).getNaziv());
		}

		// ispis podataka o mjernoim postajama
		ispisPodataka( visinaRadioSonazneMjernePostaje, mapaSenzoraMjesta);
	 ...
	 */
	private static void ispisPodataka(Integer visinaRadioSonazneMjernePostaje,
			Map<Mjesto, List<String>> mapaSenzoraMjesta) {
		
		mjernePostaje.getSorted().forEach(ispisPostaja->{
		...
		mjernePostaje.getSorted().forEach(ispisZupanija->{
		...
		mjernePostaje.getSorted().stream().filter(c->c.getNaziv().toLowerCase().charAt(0)=='m')
		.filter(c->c.getNaziv().toLowerCase().charAt(c.getNaziv().length()-1)=='r').forEach(System.out::println);

		
		int sum = mjernePostaje.getSorted().stream().mapToInt(w -> w.getNaziv().length()).sum();
		System.out.println("Suma duljina naziva: "+sum);
		
		System.out.println("Najkraci naziv ima: " + mjernePostaje.getSorted().stream()
				.min(Comparator.comparing(s -> s.getNaziv().length())));
		System.out.println("Najdulji naziv ima: " + mjernePostaje.getSorted().stream()
				.max(Comparator.comparing(s -> s.getNaziv().length())));
		
		System.out.println("Ispis drzava genericke klase:");
		drzave.getSorted().forEach(drzava->System.out.println(drzava.getNaziv() + " ima povrsinu: " + drzava.getPovrsina()));
	}
