...
public class SenzorTemperature extends Senzor {
	 ...
	 * 
	 * @param novaMjernaPostaja mjerna postaja u kojoj se mjeri temperatura
	 * @param i iterator koji odreduje redni broj mjerne postaje
	 * @exception NiskaTemperaturaException generirana vrijednost temperature je preniska
	 * @exception VisokaTemperaturaException generirana vrijednost temperature je previsoka
	 */
	public void generirajVrjednost(Object[] novaMjernaPostaja, int i) {
		int randomInt = 0;
		...
		do {
			...
			try {
				...
				System.out.println("Mjerna postaja: " + ((MjernaPostaja) novaMjernaPostaja[i]).getNaziv());
				ex.printStackTrace();
				logger.error("Pogresna temperatura postaje: " + ((MjernaPostaja) novaMjernaPostaja[i]).getNaziv(), ex);
				nastavi = true;
			} catch (VisokaTemperaturaException ex1) {
				System.out.println("Mjerna postaja: " + ((MjernaPostaja) novaMjernaPostaja[i]).getNaziv());
				ex1.printStackTrace();
				logger.error("Pogresna temperatura postaje: " + ((MjernaPostaja) novaMjernaPostaja[i]).getNaziv(), ex1);
				nastavi = true;
			}
		} while (nastavi);
	...
	/**
	 * Provjerava je li genrirani podatak unutar zadanih granica. Ako nije, baca se exception.
	 * 
	 * @param randomInt generirana vrijednost
	 * @return vraca vrijednost varijable koja kontrolira hice li se petlja zaustaviti ili ne
	 * @throws VisokaTemperaturaException baca iznimku ako je temperatra previsoka
	 * @throws NiskaTemperaturaException baca se iznimka ako je temperatura preniska
	 */
