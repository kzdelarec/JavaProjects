...
import hr.java.vjezbe.iznimke.DuplaDrzavaException;
...
public class Glavna {
	...
	public static void main(String[] args) throws InterruptedException {
		...
		Map<String, BigDecimal> mapaDrzava = new HashMap<>();
		...
		for (int i = 0; i < BROJ_POSTAJA; i++) {
			...
			do {
				try {
					provjeraDrzave(mapaDrzava, nazivDrzave, povrsinaDrzave);
					nastavi = false;
				}
				catch(DuplaDrzavaException ex){
					System.out.println("Duplo, upisite ponovno");
					logger.error("Dupli unos drzave",ex);
					nazivDrzave=unos.nextLine();
					povrsinaDrzave=unosPovrsine(unos);
					nastavi=true;
				}
			}while(nastavi);
	...
	/**
	 * Provjerava ponavlja li se upisana drzava. Ako se ponavlja, funkcija baca iznimku.
	 * 
	 * @param mapaDrzava mapa svih unesenih drzava gdje je ime drzave kljuc, a povrsina vrijednost
	 * @param nazivDrzave kljuc koji provjeravamo
	 * @param povrsinaDrzave vrijednost koju provjeravamo
	 * 
	 */
	private static void provjeraDrzave(Map<String, BigDecimal> mapaDrzava, String nazivDrzave,
			BigDecimal povrsinaDrzave) {
		if(mapaDrzava.containsKey(nazivDrzave)&& mapaDrzava.containsValue(povrsinaDrzave)) {
			throw new DuplaDrzavaException("Drzava se ponavlja!");
		}
		else {
			mapaDrzava.put(nazivDrzave, povrsinaDrzave);
		}
	}
	...
	private static Integer unosVisineRadioSonazneMjernePostaje(Scanner unos) {
		...
		Integer visina = RadioSondaznaMjernaPostaja.kontrolaVisine(unos);
