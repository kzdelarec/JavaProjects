...
public class Glavna {
	...
	public static void main(String[] args) throws InterruptedException {
		...
		List<String> popisGradova = new ArrayList<>();
		List<String> popisSela= new ArrayList<>();
		List<String> popisOstalog = new ArrayList<>();
		Map<Mjesto, List<String>> mapaSenzoraMjesta = new HashMap<>();
		Map<String, List<String>> mapaPostajaUMjestu = new HashMap<>();
		...
		for (int i = 0; i < BROJ_POSTAJA; i++) {
			...
			if (i < 2) {
				novaMjernaPostaja.add(dohvatiPostaju(novaMjernaPostaja, novoMjesto, novaGeografskaTocka, listaSenzora,
						nazivMjernePostaje, popisGradova,popisSela,popisOstalog, mapaPostajaUMjestu));
			} else {
			...
			mapaPostajaUMjestu.put("GRAD", popisGradova);
			mapaPostajaUMjestu.put("SELO", popisSela);
			mapaPostajaUMjestu.put("OSTALO", popisOstalog);

		}

		// ispis podataka o mjernoim postajama
		ispisPodataka(novaMjernaPostaja, visinaRadioSonazneMjernePostaje, mapaSenzoraMjesta,mapaPostajaUMjestu);
	 ...
	 *            lista mjernih postaja
	 * @param mjesto
	 *            objekt klase Mjesto
	 * @param geo
	 *            objekt klase GeografskaTocka
	 * @param listaSenzora
	 *            lista senzora mjerne postaje
	 * @param naziv
	 *            podatak o nazivu mjerne postaje
	 * @param popisOstalog 
	 * @param popisSela 
	 * @param popisGradova 
	 * @param mapaPostajaUMjestu 
	...
	private static MjernaPostaja dohvatiPostaju(List<MjernaPostaja> novaMjernaPostaja, Mjesto mjesto,
			GeografskaTocka geo, List<Senzor> listaSenzora, String naziv, List<String> popisGradova, List<String> popisSela, List<String> popisOstalog, Map<String, List<String>> mapaPostajaUMjestu) {
		...
		if(mjesto.getVrstaMjest().name().equals("GRAD")) {
			popisGradova.add(mjernaPostaja.getNaziv());
		}
		if(mjesto.getVrstaMjest().name().equals("SELO")) {
			popisSela.add(mjernaPostaja.getNaziv());
		}
		if(mjesto.getVrstaMjest().name().equals("OSTALO")) {
			popisOstalog.add(mjernaPostaja.getNaziv());
		}
	 ...
	 * 
	 * @param unos
	 *            podatak kojeg unosi korisnik
	 * @param novaMjernaPostaja
	 *            lista mjernih postaja
	 * @param zup
	 *            objekt klase Zupanija
	 ...
	 * senzorima. Nakon ispisa senzora, iz klase Drzava dohvaca listu zupanija te ju
	 * sortira i ispisuje. Takoder kontrolira da ne ispisuje dvije iste zupanije.
	 * Nakon ispisa zupnija ispisuje podatke o tome koja mjerna postaja ima koje
	 * senzore.
	 * 
	 * @param novaMjernaPostaja
	 ...
	 * @param mapaSenzoraMjesta
	 *            mapa u kojoj se sa mjernim postajama pove
	 * @param mapaPostajaUMjestu 
	 */
	private static void ispisPodataka(List<MjernaPostaja> novaMjernaPostaja, Integer visinaRadioSonazneMjernePostaje,
			Map<Mjesto, List<String>> mapaSenzoraMjesta, Map<String, List<String>> mapaPostajaUMjestu) {
		...
		novaMjernaPostaja.forEach(ispisZupanija -> {
			Set<Zupanija> lista = ispisZupanija.getMjesto().getZupanija().getDrzava().getPopisZupanijaDrzave();
			lista.forEach(zup -> {
		...
		while (iterator.hasNext()) {
			...
			keys.forEach(i -> {
				...
				System.out.println(
						mapaSenzoraMjesta.get(i).toString().replace("[", "").replace("]", "").replace(", ", "\n")
								+ "\n");
			});

		}
		// ----------Ispis mjernih postaja po duljini naziva
		Function<MjernaPostaja, Integer> duljina = MjernaPostaja::getDuljinaImena;
		Comparator<MjernaPostaja> poDuljini = Comparator.comparing(duljina);
		novaMjernaPostaja.sort(poDuljini);
		for (int i = 0; i < novaMjernaPostaja.size(); i++) {
			if (i == 0) {
				System.out.println(Stream.of(novaMjernaPostaja.get(i).getNaziv()));
			}
			if (i == novaMjernaPostaja.size() - 1) {
				System.out.println(Stream.of(novaMjernaPostaja.get(i).getNaziv()));
			}

		}
		novaMjernaPostaja.forEach(takePostaja->{
			BigDecimal filter = new BigDecimal(50);
			Stream.of(takePostaja.getGeografskaTocka().getX()).takeWhile(i-> i.compareTo(filter)>0).forEach(System.out::println);
		});
		
		//---------ispis mape-------
		for(String vrstaMjesta : mapaPostajaUMjestu.keySet()) {
			String key = vrstaMjesta;
			String value = mapaPostajaUMjestu.get(vrstaMjesta).toString();
